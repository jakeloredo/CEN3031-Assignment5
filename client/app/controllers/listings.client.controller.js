angular.module('listings').controller('ListingsController', ['$scope', '$location', '$stateParams', '$state', 'Listings', 
  function($scope, $location, $stateParams, $state, Listings){
      ...
      Listings.getAll().then(function(response) {
		...
		$scope.listings.forEach(function(listing){
			if(listing.coordinates){
				var marker = listing.coordinates;
				marker.id = i++;
				marker.name = listing.name;
				marker.code = listing.code;			
    ...
    $scope.update = function(isValid) {
	  ...
	  if($scope.name) listing.name = $scope.name;
	  if($scope.code) listing.code = $scope.code;
	  if($scope.address) listing.address = $scope.address;
	  
      Listings.update(id, listing)
              .then(function(response) {
                $state.go('listings.list', { successMessage: 'Listing was updated' });
              }, function(error) {
    ...
    $scope.remove = function() {
      ...
      Listings.delete(id)
              .then(function(response) {
                $state.go('listings.list', { successMessage: 'Listing was deleted' });
              }, function(error) {
                $scope.error = 'Was not able to delete the listing\n' + error;
              });
    };
